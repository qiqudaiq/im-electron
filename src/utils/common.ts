...
export const feedbackToast = (config?: FeedbackToastParams) => {
  ...
  message.open({
    ...
    duration,
    onClose,
    key: 'feedback-toast', // 使用固定key，新消息会替换旧消息
  });
  if (error) {
...
export const getResourceUrl = (path: string): string => {
  ...
  if (path.includes('/api/object/')) {
    ...
    const isElectron = Boolean(window.electronAPI); // 判断是否是Electron客户端
    
    // 根据环境和客户端类型选择不同的域名
    let baseUrl = window.location.origin; // 默认使用当前域名
    
    if (isDevelopment || isTest) {
      baseUrl = '';
    } else if (isElectron) {
      // Electron客户端使用自动寻路得到的最优服务器
      const optimalHost = ApiAutoRoute.currentHost;
      if (optimalHost) {
        baseUrl = `https://${optimalHost}`;
      } else {
        // 如果还没有寻路结果，使用默认域名
        baseUrl = '';
      }
    }
    // Web端使用当前域名（window.location.origin） 
    
    return `${baseUrl}${basePath}${newPath.split('/api/object/')[1]}`;
  }
...
export const downloadFile = async (originUrl: string) => {
  originUrl = getResourceUrl(originUrl);
