...
export const Login = () => {
  ...
  // const [loginMethod, setLoginMethod] = useState<"account" | "email">(getLoginMethod());
  const [loginMethod, setLoginMethod] = useState<"account" | "email" | "verifyCode">(
    "account",
  );
  ...
  const updateLoginMethod = useCallback(
    (method: "account" | "email" | "verifyCode") => {
  ...
  return (
    <div className="relative flex h-full flex-col">
      ...
      <div className="flex flex-1 items-center justify-center">
        <LeftBar />
        <div
          className={`${styles.login} mr-14 h-[450px] w-[350px] rounded-md p-8`}
          style={{
            boxShadow: "0 0 30px rgba(0,0,0,.1)",
            height: formType === 2 ? 850 : 450,
          }}
        >
          {formType === 0 && (
            <LoginForm
