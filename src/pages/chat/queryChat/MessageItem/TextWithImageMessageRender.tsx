...
const TextWithImageMessageRender: FC<{ message: MessageItem }> = ({ message }) => {
  ...
  const formatRecords = async (arr: []) => {
    const newArr = await Promise.all(
      arr.map(async (item: any) => {
        const { data: sendUser } = await IMSDK.getUsersInfo([item.receiver_id]);
        return {
          ...item,
  ...
  const groupClick = async () => {
    ...
    try {
      ...
      if (response.errCode === 0 && response.data) {
        ...
        const youReceive =
          formattedRecords.find((item: any) => item.receiver_id === selfID)?.amount ||
          "0";
  ...
  const groupReceive = async () => {
    try {
      ...
      if (
        ...
        const receiveRes = (await ReceiveTransfer(transferId)) as TransferResponse;
  ...
  const getStatusText = () => {
    if (transferStatus === "completed") {
      if (isTransferInitMessage) {
        return t("redPacket.hasBeenReceived");
      }
      return t("redPacket.received");
    }
    if (isExpired) {
      return t("redPacket.expired");
    }
  ...
  return (
    <div className="transferCard_container">
      <Card
        ...
        onClick={isGroup ? groupClick : handleReceiveTransfer}
        loading={isLoading}
      >
        <div className="transferCard_box">
          ...
          <div className="transferCard_right">
            <div className="transferCard_right_top">
              {isGroup && <span>{mark ?? t('placeholder.WishYouWealth')}</span>}
              {!isGroup && (
...
export default TextWithImageMessageRender;
