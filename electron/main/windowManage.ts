...
export function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      enableBlinkFeatures: 'CSSColorSchemeUARendering,DesktopCaptureChangeSource',
    },
  });
  // mainWindow.webContents.openDevTools(); // 加在这里
  sdkInstance = initIMSDK(mainWindow.webContents);

  // 设置媒体权限处理
  mainWindow.webContents.session.setPermissionRequestHandler((webContents, permission, callback) => {
    console.log(`[Electron] Permission requested: ${permission}`);
    callback(true); // Allow all media related permissions
  });
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    // 生产环境 - 直接加载登录页
    mainWindow.loadFile(global.pathConfig.indexHtml, { hash: '/login' });
  }
